...
def main(args):
    ...
    cudnn.benchmark = True # True for fast training, False for reproducibility
    ...
    print("Model = %s" % str(model_without_ddp)) # print model architecture

    eff_batch_size = args.batch_size * args.accum_iter * misc.get_world_size() # effective batch size
    ...
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu], find_unused_parameters=True) #wrap model with DDP
        model_without_ddp = model.module # unwrap model from DDP wrapper

    # following timm: set wd as 0 for bias and norm layers
    param_groups = optim_factory.add_weight_decay(model_without_ddp, args.weight_decay) # add weight decay to all parameters
